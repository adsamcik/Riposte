...
class ImportZipBundleUseCaseTest {
    ...
    @Test
    fun `returns error when not a valid meme zip bundle`() = runTest {
    ...
    @Test
    fun `imports extracted memes successfully`() = runTest {
        val zipUri = mockk<Uri>()
    ...
    fun `handles import failures gracefully`() = runTest {
        ...
        coEvery { repository.importImage(imageUri, null) } returns Result.failure(
    ...
    @Test
    fun `includes extraction errors in result`() = runTest {
        val zipUri = mockk<Uri>()

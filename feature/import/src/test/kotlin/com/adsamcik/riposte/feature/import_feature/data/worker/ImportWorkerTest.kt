...
class ImportWorkerTest {
    ...
    private fun createMeme(id: Long = 1L) = Meme(
        id = id,
    ...
    @Test
    fun `doWork returns failure when request id is missing`() = runTest {
    ...
    @Test
    fun `doWork returns failure when request not found in database`() = runTest {
    ...
    @Test
    fun `doWork processes all pending items successfully`() = runTest {
    ...
    @Test
    fun `doWork handles import failure for individual items`() = runTest {
        val request = createRequest()
        val items = listOf(
            createItem(id = "item-1", fileName = "meme1.png"),
            createItem(id = "item-2", fileName = "meme2.png"),
        ...
        coEvery { importRequestDao.getRequest("req-1") } returns request
        coEvery { importRequestDao.getPendingItems("req-1") } returns items
        coEvery { importRepository.importImage(any(), any()) } returnsMany listOf(
    ...
    @Test
    fun `doWork marks request as completed when at least one item succeeds`() = runTest {
        val request = createRequest()
        val items = listOf(
            createItem(id = "item-1", fileName = "meme1.png"),
            createItem(id = "item-2", fileName = "meme2.png"),
    ...
    @Test
    fun `doWork marks request as failed when all items fail`() = runTest {
        val request = createRequest()
        val items = listOf(
            createItem(id = "item-1", fileName = "meme1.png"),
            createItem(id = "item-2", fileName = "meme2.png"),
    ...
    @Test
    fun `doWork creates notification channel`() = runTest {
        val request = createRequest()
        coEvery { importRequestDao.getRequest("req-1") } returns request
    ...
    @Test
    fun `doWork cleans up old requests`() = runTest {
        val request = createRequest()
        coEvery { importRequestDao.getRequest("req-1") } returns request
        coEvery { importRequestDao.getPendingItems("req-1") } returns emptyList()
    ...
    @Test
    fun `doWork shows completion notification when in background`() = runTest {
    ...
    @Test
    fun `doWork does not show notification when in foreground`() = runTest {
    ...
    @Test
    fun `doWork resumes from previously completed items`() = runTest {
    ...
    @Test
    fun `doWork cleans up staging directory`() = runTest {
    ...
    @Test
    fun `doWork passes correct metadata to importImage`() = runTest {
    ...
    @Test
    fun `doWork updates progress for each item`() = runTest {

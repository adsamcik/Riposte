...
class SearchDelegateTest {
    ...
    @Test
    fun `init loads recent searches`() = runTest(testDispatcher) {
    ...
    @Test
    fun `init filters out internal query syntax from recent searches`() = runTest(testDispatcher) {
        every { searchUseCases.getRecentSearches() } returns flowOf(
    ...
    @Test
    fun `updateQuery updates query in state`() = runTest(testDispatcher) {
        val scope = createDelegateScope()
        delegate.init(scope)
        advanceUntilIdle()
    ...
    @Test
    fun `search is debounced and performs hybrid search`() = runTest(testDispatcher) {
        val scope = createDelegateScope()
        delegate.init(scope)
        advanceUntilIdle()
    ...
    @Test
    fun `selectRecentSearch performs immediate search`() = runTest(testDispatcher) {
        val scope = createDelegateScope()
        delegate.init(scope)
        advanceUntilIdle()
    ...
    @Test
    fun `deleteRecentSearch removes from state`() = runTest(testDispatcher) {
        val scope = createDelegateScope()
        delegate.init(scope)
        advanceUntilIdle()
    ...
    @Test
    fun `clearRecentSearches empties the list`() = runTest(testDispatcher) {
        val scope = createDelegateScope()
        delegate.init(scope)
        advanceUntilIdle()
    ...
    @Test
    fun `refilter applies emoji filters to existing results`() = runTest(testDispatcher) {
        val scope = createDelegateScope()
        delegate.init(scope)
        advanceUntilIdle()
    ...
    @Test
    fun `search error sets errorMessage`() = runTest(testDispatcher) {
    ...
    @Test
    fun `falls back to text search on UnsatisfiedLinkError`() = runTest(testDispatcher) {
    ...
    @Test
    fun `blank query clears results`() = runTest(testDispatcher) {
        val scope = createDelegateScope()
        delegate.init(scope)
        advanceUntilIdle()

...
class ImportRequestDaoTest {
    ...
    @Test
    fun `insertRequest and getRequest returns entity`() =
        runTest {
    ...
    @Test
    fun `getRequest returns null for unknown id`() =
        runTest {
    ...
    @Test
    fun `getActiveRequests returns only pending and in_progress`() =
        runTest {
            dao.insertRequest(
                createRequest(id = "pending", status = ImportRequestEntity.STATUS_PENDING, createdAt = 1000L),
            )
            dao.insertRequest(
                createRequest(id = "in-progress", status = ImportRequestEntity.STATUS_IN_PROGRESS, createdAt = 2000L),
            )
            dao.insertRequest(
                createRequest(id = "completed", status = ImportRequestEntity.STATUS_COMPLETED, createdAt = 3000L),
            )
            dao.insertRequest(
                createRequest(id = "failed", status = ImportRequestEntity.STATUS_FAILED, createdAt = 4000L),
            )
    ...
    @Test
    fun `getActiveRequests orders by createdAt desc`() =
        runTest {
            dao.insertRequest(
                createRequest(id = "oldest", status = ImportRequestEntity.STATUS_PENDING, createdAt = 1000L),
            )
            dao.insertRequest(
                createRequest(id = "newest", status = ImportRequestEntity.STATUS_PENDING, createdAt = 3000L),
            )
            dao.insertRequest(
                createRequest(id = "middle", status = ImportRequestEntity.STATUS_IN_PROGRESS, createdAt = 2000L),
            )
    ...
    @Test
    fun `getPendingItems returns only pending items`() =
        runTest {
    ...
    @Test
    fun `getAllItems returns all items for request`() =
        runTest {
    ...
    @Test
    fun `insertItems inserts multiple items`() =
        runTest {
            dao.insertRequest(createRequest())
            val items =
                listOf(
    ...
    @Test
    fun `updateItemStatus updates status and error message`() =
        runTest {
            dao.insertRequest(createRequest())
    ...
    @Test
    fun `updateItemStatus with null error message`() =
        runTest {
            dao.insertRequest(createRequest())
            dao.insertItems(listOf(createItem(id = "item-1")))
    ...
    @Test
    fun `updateRequestProgress updates all fields`() =
        runTest {
            dao.insertRequest(createRequest())
    ...
    @Test
    fun `insertRequest with REPLACE updates existing`() =
        runTest {
    ...
    @Test
    fun `cleanupOldRequests deletes completed and failed before cutoff`() =
        runTest {
            dao.insertRequest(
                createRequest(id = "old-completed", status = ImportRequestEntity.STATUS_COMPLETED, updatedAt = 1000L),
            )
            dao.insertRequest(
                createRequest(id = "old-failed", status = ImportRequestEntity.STATUS_FAILED, updatedAt = 1500L),
            )
            dao.insertRequest(
                createRequest(
                    id = "recent-completed",
    ...
    @Test
    fun `cleanupOldRequests preserves active requests`() =
        runTest {
            dao.insertRequest(
                createRequest(id = "old-pending", status = ImportRequestEntity.STATUS_PENDING, updatedAt = 500L),
            )
            dao.insertRequest(
                createRequest(
                    id = "old-in-progress",
    ...
    @Test
    fun `cleanupOldRequestItems deletes items of old requests`() =
        runTest {
            dao.insertRequest(
                createRequest(id = "old-req", status = ImportRequestEntity.STATUS_COMPLETED, updatedAt = 1000L),
            )
            dao.insertRequest(
                createRequest(id = "active-req", status = ImportRequestEntity.STATUS_PENDING, updatedAt = 500L),
            )
            dao.insertItems(
                listOf(

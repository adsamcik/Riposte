...
class XmpMetadataHandler
    ...
    constructor(
        ...
        private fun extractDcValue(
            xmpData: String,
            property: String,
        ): String? {
            val regex =
                Regex("""<dc:$property>\s*<rdf:Alt>\s*<rdf:li[^>]*>([^<]+)</rdf:li>""", RegexOption.DOT_MATCHES_ALL)
            return regex.find(xmpData)?.groupValues?.get(1)?.takeIf { it.isNotBlank() }
        }
        ...
        private fun extractEmojis(xmpData: String): List<String> {
            val blockRegex =
                Regex(
                    """<$XMP_PREFIX:emojis>\s*<rdf:Bag>(.*?)</rdf:Bag>\s*</$XMP_PREFIX:emojis>""",
        ...
        private fun extractTags(xmpData: String): List<String> {
            val blockRegex =
                Regex(
                    """<$XMP_PREFIX:tags>\s*<rdf:Bag>(.*?)</rdf:Bag>\s*</$XMP_PREFIX:tags>""",
                    RegexOption.DOT_MATCHES_ALL,
                )
            val blockMatch = blockRegex.find(xmpData) ?: return emptyList()
        ...
        private fun generateXmpPacket(metadata: MemeMetadata): String {
            return buildString {
                ...
                // Schema version
                appendLine(
                    """      <$XMP_PREFIX:schemaVersion>${escapeXml(
                        metadata.schemaVersion,
                    )}</$XMP_PREFIX:schemaVersion>""",
                )

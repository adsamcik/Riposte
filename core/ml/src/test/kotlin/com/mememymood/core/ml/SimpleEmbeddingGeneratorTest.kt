...
class SimpleEmbeddingGeneratorTest {
    ...
    @Test
    fun `generateFromText is case insensitive`() = runTest {
    ...
    @Test
    fun `generateFromText trims whitespace`() = runTest {
    ...
    @Test
    fun `generateFromText produces consistent results across multiple calls`() = runTest {
    ...
    @Test
    fun `generateFromText handles empty string`() = runTest {
    ...
    @Test
    fun `generateFromText handles single word`() = runTest {
        val text = "test"
    ...
    @Test
    fun `identical texts with repeated words produce same embedding`() = runTest {
    ...
    @Test
    fun `generateFromImage with no labels produces zero embedding`() = runTest {
        val mockBitmap = mockk<Bitmap>(relaxed = true)
    ...
    @Test
    fun `generateFromImage normalizes label text to lowercase`() = runTest {
        val mockBitmap = mockk<Bitmap>(relaxed = true)
        val mockLabelsUpperCase = listOf(
            createMockImageLabel("CAT", 0.9f),
            createMockImageLabel("ANIMAL", 0.8f)
        )
        ...
        val mockLabelsLowerCase = listOf(
            createMockImageLabel("cat", 0.9f),
            createMockImageLabel("animal", 0.8f)
        )
    ...
    @Test
    fun `generateFromUri returns zero embedding for null bitmap`() = runTest {
    ...
    @Test
    fun `isReady returns true`() = runTest {
        assertThat(generator.isReady()).isTrue()
    }

    @Test
    fun `initialize completes without error`() = runTest {
        // Should not throw
    ...
    private fun createMockImageLabel(text: String, confidence: Float): ImageLabel {

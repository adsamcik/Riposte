...
class MediaPipeEmbeddingGeneratorTest {
    ...
    @Test
    fun `generateFromText produces normalized embedding with magnitude approximately 1`() =
        runTest {
    ...
    @Test
    fun `all embedding values are within valid range of -1 to 1`() =
        runTest {
    ...
    @Test
    fun `identical text produces bit-exact identical embeddings across calls`() =
        runTest {
    ...
    @Test
    fun `synonyms produce highly similar embeddings - funny cat vs hilarious feline`() =
        runTest {
    ...
    @Test
    fun `sad dog and depressed puppy produce similar embeddings`() =
        runTest {
            val text1 = "sad dog"
    ...
    @Test
    fun `happy and joyful produce similar embeddings`() =
        runTest {
    ...
    @Test
    fun `context-aware similarity - king and queen are more similar than king and banana`() =
        runTest {
            val text1 = "king"
            val text2 = "queen"
            val text3 = "banana"
    ...
    @Test
    fun `paraphrase detection - different wording same meaning produces similar embeddings`() =
        runTest {
    ...
    @Test
    fun `semantic triangle inequality holds for related concepts`() =
        runTest {
    ...
    @Test
    fun `meme search - laugh emoji should match funny content`() =
        runTest {
            val queryEmoji = "😂"
    ...
    @Test
    fun `meme categories - fire emoji matches hot and trendy content`() =
        runTest {
            val fireEmoji = "🔥"
    ...
    @Test
    fun `generateFromText handles various emojis`() =
        runTest {
    ...
    @Test
    fun `generateFromText handles emoji sequences and zwj emojis`() =
        runTest {
            val complexEmojis =
                listOf(
    ...
    @Test
    fun `generateFromText handles single character`() =
        runTest {
            val text = "a"
            setupMockEmbedderForText(text, createNormalizedEmbedding())
    ...
    @Test
    fun `generateFromText handles mixed content - text with emojis and special chars`() =
        runTest {
    ...
    @Test
    fun `isReady returns true after successful initialization`() =
        runTest {
    ...
    @Test
    fun `initialize can be called multiple times without error`() =
        runTest {
            setupSuccessfulInitialization()
    ...
    @Test
    fun `close releases resources and marks as not ready`() =
        runTest {
            setupSuccessfulInitialization()
            generator.initialize()
    ...
    @Test
    fun `close can be called multiple times without error`() =
        runTest {
            setupSuccessfulInitialization()
            generator.initialize()
    ...
    @Test
    fun `generateFromText throws when model not initialized`() =
        runTest {
            val uninitializedGenerator = createUninitializedGenerator()
    ...
    @Test
    fun `initialize throws meaningful error when model file is missing`() =
        runTest {
    ...
    @Test
    fun `generateFromText handles model inference errors gracefully`() =
        runTest {
    ...
    @Test
    fun `generateFromImage with no labels produces valid embedding`() =
        runTest {
            val mockBitmap = mockk<Bitmap>(relaxed = true)
    ...
    @Test
    fun `generateFromUri handles null bitmap`() =
        runTest {
    ...
    @Test
    fun `generateFromUri handles IOException when reading stream`() =
        runTest {
            val mockUri = mockk<Uri>()
    ...
    @Test
    fun `embedding generation is deterministic across instance recreation`() =
        runTest {
    ...
    private fun createSemanticEmbedding(
        ...
        val embedding =
            FloatArray(testEmbeddingDimension) { i ->
     ...
     */
    private fun setupMockEmbedderForText(
        text: String,

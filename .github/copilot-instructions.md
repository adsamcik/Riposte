...
- **Thread-safe caches**: Any `MutableMap`/`MutableList` shared across coroutines MUST be guarded with `Mutex` (suspend functions) or `synchronized` (non-suspend functions). See `kotlin.instructions.md` for the pattern.
...
- **Flow error handling**: Use `.catch {}` before `.collect {}` on Flow observers in ViewModels to prevent silent failures. See `hilt.instructions.md`.

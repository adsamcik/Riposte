 ...
 * fixed symbolic strings. This avoids any need for a separate document index.
 *
 * Hilt note: ContentProviders are created before `Application.onCreate`, so
 * field-injection isn't available. We pull the [MemeDao] lazily on first use
...
class RiposteDocumentsProvider : NullSafeSearchDocumentsProvider() {
    ...
    override fun queryChildDocuments(
        ...
        when (parentDocumentId) {
            ...
            DOC_ALL -> populateMemes(cursor) { runBlocking { daoLazy.getAllMemes().first() } }
            DOC_FAVORITES -> populateMemes(cursor) { runBlocking { daoLazy.getFavoriteMemes().first() } }
            DOC_RECENT -> populateMemes(cursor) {
                runBlocking { daoLazy.getRecentlyViewedMemes(RECENT_LIMIT).first() }
            }
            else -> {
                // Meme leaf nodes have no children — return empty cursor
            }
        }
        return cursor
    }
    ...
    override fun openDocumentThumbnail(
        ...
        // Most pickers ask for thumbnails; returning the full image is
        // acceptable for our size range (typically <500KB after import
        // resize). Real thumbnailing would call into Coil or BitmapFactory
        // here but isn't worth the complexity for v1.
